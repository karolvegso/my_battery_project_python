...
path_to_save_meas_data="C:/Users/User/Desktop/Karol/keithley_6221_elcurrent_source_2182_nanovoltmeter/Cu_ITO_LPSCl_Li_04/"
...
keithley6221_inst = rm.open_resource('TCPIP0::147.213.112.150::1394::SOCKET')
...
keithley6221_inst.write('CURRent -1.0e-5')
...
keithley_inst = rm.open_resource('ASRL4::INSTR')
...
for index_0 in range(no_cycles):
    ...
    while (ocp_event_counter < ocp_event_counter_threshold):
        ...
        if (float(volt_value) < volt_threshold):
            #print('Dont increase ocp event counter.')
            ocp_event_counter=ocp_event_counter
        else:
            # increase of OCP event counter
            #print('Increase ocp event counter.')
            ocp_event_counter+=1
